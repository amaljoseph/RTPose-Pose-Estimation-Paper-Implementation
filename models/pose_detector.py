...
class PoseDetector(object):
    ...
    def pad_image(self, img, stride, pad_value):
        ...
        pad = [0] * 2
        pad[0] = (stride - (h % stride)) % stride
        pad[1] = (stride - (w % stride)) % stride
...
if __name__ == '__main__':
    ...
    pose_detector = PoseDetector(posenet, args.weights, device=args.gpu, precise=args.precise)


    img1 = cv2.imread(args.img)
    cv2.imshow("original", img1)


    poses, _ = pose_detector(img1)


    img2 = draw_person_pose(img1, poses)
    print('Saving result into result.png...')
    cv2.imwrite('results/result.png', img2)
    ...
    cv2.waitKey(0)
    cv2.imwrite('results/result0.png', res_img)
    """
